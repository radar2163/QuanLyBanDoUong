...
public class CustomerProductAdapter extends RecyclerView.Adapter<CustomerProductAdapter.ProductViewHolder> {
    ...
    public CustomerProductAdapter(Context context, List<Product> productList, OnProductClickListener listener) {
    ...
    public void onBindViewHolder(@NonNull ProductViewHolder holder, int position) {
        ...
        // Hiển thị số lượng còn lại (ẩn hoặc hiển thị nhỏ)
        if (product.getQuantity() > 0) {
            holder.productQuantity.setText("Còn lại: " + product.getQuantity());
            holder.productQuantity.setVisibility(View.GONE); // Ẩn để giao diện gọn hơn
        ...
        //  Load ảnh bằng Glide (xử lý cả URL, URI, drawable)
        ...
        holder.btnFavorite.setOnClickListener(v -> {
            ...
            // Thông báo cho listener biết có thay đổi yêu thích
    ...
    private void updateFavoriteIcon(ImageView icon, boolean isFavorite) {
        if (isFavorite) {
            ...
            icon.setColorFilter(android.graphics.Color.parseColor("#FF6B6B")); // Màu đỏ khi yêu thích
        } else {
            icon.setImageResource(R.drawable.ic_favorite_outline);
            icon.setColorFilter(android.graphics.Color.parseColor("#CCCCCC")); // Màu xám khi chưa yêu thích
        }
    }
}
